...
abe028=next((t for t in q.get("queue",[]) if t.get("id")=="ABE-028"),None)
if not abe028 or abe028.get("status") not in {"READY","COMPLETE_FOUNDATION"} or abe028.get("safe_autonomous") is not True: raise SystemExit("ABE-028 not authoritative validated state")
