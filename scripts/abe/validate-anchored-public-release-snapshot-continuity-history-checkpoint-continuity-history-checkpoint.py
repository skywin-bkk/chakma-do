...
CHECKPOINT = ROOT / "build/abe/anchored-public-release-snapshot-continuity-history-checkpoint-continuity-history-checkpoint.json"
...
if history.get("publication_policy") != "explicit-public-only" or history.get("classification_boundary") != "PUBLIC_ONLY" or any(history.get(k) is not False for k in flags):
    raise SystemExit("unsafe/non-PUBLIC ABE-035 history")
...
terminal_abe032_sequence = history.get("terminal_history_sequence")
for value, label in ((history_sequence, "ABE-035 terminal history sequence"), (continuity_sequence, "ABE-034 continuity sequence"), (terminal_abe032_sequence, "ABE-032 terminal history sequence")):
    if not isinstance(value, int) or isinstance(value, bool) or value < 1:
        raise SystemExit(f"invalid {label}")

identities = {
    "current_continuity_identity": history.get("current_continuity_identity"),
    "current_checkpoint_identity": history.get("current_checkpoint_identity"),
    "terminal_abe032_history_identity": history.get("terminal_history_identity"),
    "current_anchor_continuity_identity": history.get("current_anchor_continuity_identity"),
    "current_anchor_identity": history.get("current_anchor_identity"),
}
if any(not valid_identity(value) for value in identities.values()):
    raise SystemExit("invalid chained identity")

if checkpoint.get("schema_version") != 1 or checkpoint.get("stage") != "ABE-036" or checkpoint.get("checkpoint_state") != "SEALED":
    raise SystemExit("invalid ABE-036 checkpoint")
expected = {
    ...
    "current_continuity_identity": identities["current_continuity_identity"],
    "current_checkpoint_identity": identities["current_checkpoint_identity"],
    "terminal_abe032_history_sequence": terminal_abe032_sequence,
    "terminal_abe032_history_identity": identities["terminal_abe032_history_identity"],
    "current_anchor_continuity_identity": identities["current_anchor_continuity_identity"],
    "current_anchor_identity": identities["current_anchor_identity"],
...
print(f"ABE-036 checkpoint validation PASS: terminal_history_sequence={history_sequence}; state=SEALED; deployment_authorized=false")
