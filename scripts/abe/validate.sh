...
required=("README.md" ".gitignore" "docs/ABE/README.md" "docs/ABE/BUILD_STATUS.md" ".github/pull_request_template.md" "config/abe/build-plan.json" "config/abe/task-queue.json" "config/abe/department-manifest.json" "config/abe/public-allowlist.json" "scripts/abe/build-public-manifest.py" "scripts/abe/discover-department-evidence.py" "scripts/abe/select-next-task.py" "scripts/abe/runner.py" "scripts/abe/plan-execution.py" "scripts/abe/write-audit-ledger.py" "scripts/abe/write-execution-receipt.py" "scripts/abe/propose-transition.py" "scripts/abe/bounded-work-loop.py" "scripts/abe/validate-public-registry.py")
...
assert loop['status']=='RUNNING' and loop['bounded_max_steps']==5; assert loop['next_task']==state['selected_task']; assert loop['authoritative_department_scope']==['DO-DEP-01','DO-DEP-14'] and loop['verified_baseline']=='DO-DEP-04'; assert loop['production_deploy'] is False and loop['destructive_actions'] is False and loop['external_writes'] is False; assert not Path('build/abe/work-loop.lock').exists(); print('ABE runner, evidence chain, bounded work-loop, and controlled transition proposal: PASS')
PY
